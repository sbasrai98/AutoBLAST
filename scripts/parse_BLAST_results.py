...
def filter_hits(blast_results_file: str, filter_keyword: str) -> pd.DataFrame:
    '''Parse BLAST results file and generate a human-readable .tsv file containing viral contigs and relevant columns.
    ...
    keep_cols = ['qseqid', 'sseqid', 'pident', 'length', 'qlen', 'slen', 'sscinames', 'scomnames', 'sblastnames', 'sskingdoms', 'stitle']
    results = results[keep_cols]
    results = results.drop_duplicates(subset=['qseqid'])
    results = results[(results['sscinames'] == filter_keyword) |
                (results['scomnames'] == filter_keyword) |
                (results['sblastnames'] == filter_keyword) |
                (results['sskingdoms'] == filter_keyword)]
    results.sort_values(['sscinames', 'pident', 'length'], ascending= [True, False, False], inplace=True)
    results = results[ ['qseqid', 'sscinames', 'pident', 'length', 'qlen', 'slen', 'sseqid', 'stitle'] ]
    contig_num = re.compile('_[0-9]+') # get contig numbers. Expects headers like ">NODE_562_length.."
